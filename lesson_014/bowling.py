...
class FrameSumError(Exception):
    ...
    def __str__(self):
        return f'Ошибка в записи фрейма "{self.f}"; сумма должна быть <= 10'
...
def get_score(game_result=None, mode='private'):
    ...
    # Для удобства подсчёта очков: заменяем фрейм страйка 'X' на фрейм '0X', промах по кеглям '-' заменяем на '0'
    result = game_result.upper().replace('Х', 'X').replace('X', '0X').replace('-', '0')
